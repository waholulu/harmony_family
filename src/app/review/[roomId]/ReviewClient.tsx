...
import type { RevealData } from "@/lib/ai-client";

export default function ReviewClient(props: { params: Promise<{ roomId: string }> }) {
  const params = use(props.params);
  const router = useRouter();

  const { topic, data } = useMemo(() => {
    if (typeof window === "undefined") return { topic: "", data: null as RevealData | null };
    ...
    if (!savedReveal) return { topic: savedTopic, data: null as RevealData | null };

    try {
      return { topic: savedTopic, data: JSON.parse(savedReveal) as RevealData };
    } catch (error) {
      console.error(error);
      return { topic: savedTopic, data: null as RevealData | null };
    }
  }, [params.roomId]);
    ...
    <div className="min-h-screen bg-zinc-900 text-zinc-50 flex flex-col items-center justify-center p-4 py-12">
      <div className="max-w-md w-full animate-in fade-in slide-in-from-bottom-8 duration-700 space-y-8">
        ...
        <div className="flex gap-4 pt-4">
          <Button onClick={handleRestart} className="w-full bg-white hover:bg-zinc-200 text-zinc-900 font-medium h-12">
